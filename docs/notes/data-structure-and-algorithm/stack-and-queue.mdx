...
import StackSvg from "./images/stack.png";
...
<Figure src={StackSvg} caption="圖片來源：維基百科" />
...
Queue 存取資料的順序是採取先進先出的方式，也就是 **first in first out(FIFO)**。將資料存入 Queue 裡面稱為 `enqueue`，將資料從 Queue 中取出稱為 `dequeue`。
